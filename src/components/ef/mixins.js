...
export let jsplumbSetting = reactive({
  // 动态锚点、位置自适应
  ...
  Connector: ['Flowchart', { stub: 10, gap: 1, alwaysRespectStubs: false, midpoint: 0, cornerRadius: 10 }],
  ...
  RenderMode: 'svg',
  // 鼠标滑过线的样式
  HoverPaintStyle: { stroke: '#7faaee', strokeWidth: 2 },
  // 滑过锚点效果
 ...
 * 连线参数
 */
export let jsplumbConnectOptions = reactive({
  isSource: true,
  isTarget: true,
 ...
 * 源点配置参数
 */
export let jsplumbSourceOptions = reactive({
...
export let jsplumbSourceOptions2 = reactive({
  // 设置可以拖拽的类名，只要鼠标移动到该类名上的DOM，就可以拖拽连线
  filter: '.flow-node-drag',
  filterExclude: false,
  // anchor: 'Continuous',
...
export let jsplumbTargetOptions = reactive({
  // 设置可以拖拽的类名，只要鼠标移动到该类名上的DOM，就可以拖拽连线
  filter: '.flow-node-drag',
  filterExclude: false,
  anchor: 'Continuous',
